...
# loop to start from where the code left off, remove if starting from scratch
for coe in coes:
    tot_dfs[coe] = pd.read_csv('data/COE/test monthly counts ' + coe + ' categories 2022-2023.csv', index_col=0)
...
for idx, df in enumerate(pd.read_csv('data/2023_update/AIR Datapull Expanded.csv', chunksize=10000)):
    ...
    #if idx > -1:
    if idx > 566:
        with open('working/01coe_tracker.txt', 'w') as file:
            file.write(str(idx))
            file.close()
        ...
        for coe in coes:
            ...
            dates = [int(str(y) + str(m).zfill(2)) for y in range(2022, 2024) for m in range(1, 13)]
            for yyyymm in dates:
            ...
            if tot_dfs[coe] is None:
                ...
                tot_dfs[coe].to_csv('data/COE/test monthly counts '+ coe + ' categories 2022-2023.csv')
            pass
