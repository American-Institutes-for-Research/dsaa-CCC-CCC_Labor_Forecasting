...
for i, f in enumerate(my_list):
    if i > -1 and '.csv.gz' in f:
        ...
        skilldf = df[['ID', 'POSTED', 'EXPIRED_MONTH', "EXPIRED_YEAR", 'EXPIRED',
                      'POSTED_MONTH', "POSTED_YEAR", 'SKILLS_NAME','COUNTY_NAME']].copy()
        ...
        for county in county_names:
            ...
            if i == 0:
                ...
                if county in tot_dfs.keys():
                    tot_df = tot_dfs[county]
                else:
                    tot_df = pd.read_csv('data/test monthly counts ' + county + '.csv',index_col=0)
                    tot_df.index = cdf.index
            ...
            tot_dfs[county] = tot_df
            tot_df.to_csv('data/test monthly counts '+county+'.csv')
            pass
