...
input:
    emsi_skills_api/EMSI_skills_with_categories.xlsx - skill category crosswalk
    data/us_postings/*.csv.gz - raw job postings data obtained from EMSI
...
for idx, f in enumerate(my_list):
    if idx > -1 and '.csv.gz' in f:
        ...
        cdf = pd.DataFrame(columns=skill_cat_cols)
