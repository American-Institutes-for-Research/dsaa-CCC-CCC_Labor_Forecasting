...
def run_VAR_loop(result_log = None, pred_df = None, start_val= 0, max_lags = 12, test_tvalues = 5,
                        ...
                        , custom_raw_path = None, save_models = False):
    '''
    params:
    ...
    if hierarchy_lvl == 'skill':
        ...
        # 7-67 filter is to remove months with 0 obs
    ...
    for n,t in enumerate(targets):
        ...
        # we can predict a maximum of time steps into the future equal to the time steps of past data,
        # so we will forecast length equal to this number minus the number of time steps used as input length
        fc = model_fitted.forecast(y=forecast_input, steps= months_of_data - input_len_used)

        # create forecasted date index; our forecast starts at the first time step of the test data set and extends
        min_date = datetime.date(2023, 8, 1) + relativedelta(months=-test_tvalues)
